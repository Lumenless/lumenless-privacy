...
export async function deposit({ lightWasm, storage, keyBasePath, publicKey, connection, amount_in_lamports, encryptionService, transactionSigner, referrer, signer, recipientUtxoPubkey, recipientEncryptionService }: DepositParams): Promise<{ tx: string, paymentLink?: PaymentLinkData }> {
    // check limit
    ...
    // Create a UTXO keypair that will be used for inputs (depositor's keypair)
    const utxoKeypair = new UtxoKeypair(utxoPrivateKey, lightWasm);
    
    // Determine the keypair for OUTPUT UTXOs
    // If recipientUtxoPubkey is provided, create a pubkey-only keypair for the recipient
    // Otherwise, use the depositor's keypair (normal deposit mode)
    const isPaymentLink = !!recipientUtxoPubkey;
    const outputKeypair = isPaymentLink 
        ? UtxoKeypair.fromPubkeyOnly(recipientUtxoPubkey)
        : utxoKeypair;
    
    if (isPaymentLink) {
        logger.debug('Payment link mode: Using recipient pubkey for output UTXOs');
        logger.debug(`Recipient pubkey: ${outputKeypair.pubkey.toString()}`);
    } else {
        logger.debug('Using wallet-derived UTXO keypair for deposit');
    }

    // Fetch existing UTXOs for this user
    // In payment link mode, we skip consolidation and always do a fresh deposit
    logger.debug('\nFetching existing UTXOs...');
    const existingUnspentUtxos = isPaymentLink 
        ? [] // Skip consolidation for payment links
        : await getUtxos({ connection, publicKey, encryptionService, storage });
    ...
    // In payment link mode, use the recipient's keypair for outputs
    // In normal mode, use the depositor's keypair
    const outputs = [
        new Utxo({
            lightWasm,
            amount: outputAmount,
            keypair: outputKeypair,
    ...
    // If recipient's encryption service is provided, use it so recipient can see the UTXO in their balance
    logger.debug('\nEncrypting UTXOs with keypair data...');
    let encryptedOutput1: Buffer;
    let encryptedOutput2: Buffer;
    
    if (isPaymentLink && recipientEncryptionService) {
        // Use recipient's encryption service for direct balance visibility
        // This uses the same symmetric encryption format that the recipient's balance check expects
        logger.debug('Using recipient encryption service for UTXO encryption');
        encryptedOutput1 = recipientEncryptionService.encryptUtxo(outputs[0]);
        encryptedOutput2 = recipientEncryptionService.encryptUtxo(outputs[1]);
    } else {
        // Use payer's encryption key (traditional flow)
    ...
    // Skip this test in payment link mode since payer can't decrypt recipient's UTXOs
    if (!isPaymentLink) {
        logger.debug('\n=== TESTING DECRYPTION ===');
        ...
        logger.debug('\n=== SKIPPING DECRYPTION TEST (payment link mode) ===');
        logger.debug('UTXOs are encrypted for recipient - payer cannot decrypt');
    }
    ...
    while (true) {
        ...
        if (resJson.exists) {
            ...
            // If this is a payment link deposit, return the data needed to claim it
            // The recipient uses this data along with their own UTXO private key to withdraw
            if (isPaymentLink) {
                const paymentLink: PaymentLinkData = {
                    blinding: outputs[0].blinding.toString(),
                    amount: parseInt(outputAmount),
                    index: currentNextIndex,
                    mintAddress: outputs[0].mintAddress,
                };
                logger.debug('Payment link created:', JSON.stringify(paymentLink, null, 2));
                return { tx: signature, paymentLink };
            }
