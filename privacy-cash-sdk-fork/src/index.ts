...
import { deposit, PaymentLinkData } from './deposit.js';
...
import { withdraw, withdrawFromPaymentLink } from './withdraw.js';
...
// Re-export PaymentLinkData type for external use
...
export class PrivacyCash {
     ...
     */
    async getReceivingPubkey(): Promise<string> {
        const lightWasm = await WasmFactory.getInstance();
        const utxoPrivateKey = this.encryptionService.getUtxoPrivateKeyV2();
        const utxoKeypair = new UtxoKeypair(utxoPrivateKey, lightWasm);
        return utxoKeypair.pubkey.toString();
    }
    ...
    getPayLinkPublicKey(): string {
        return this.encryptionService.getPayLinkPublicKey();
    }
     ...
     */
    async getPayLinkData(): Promise<{ utxoPubkey: string; encryptionPublicKey: string }> {
        const utxoPubkey = await this.getReceivingPubkey();
        const encryptionPublicKey = this.getPayLinkPublicKey();
        return { utxoPubkey, encryptionPublicKey };
    }
     ...
     */
    async deposit({ lamports, recipientUtxoPubkey }: {
        lamports: number,
        recipientUtxoPubkey?: string
    }): Promise<{ tx: string, paymentLink?: PaymentLinkData }> {
        this.isRuning = true
    ...
    */
     ...
     */
    async claimPaymentLink({ paymentLink, recipientAddress, referrer }: {
        paymentLink: PaymentLinkData,
        recipientAddress?: string,
        referrer?: string
    }) {
        this.isRuning = true
        logger.info('claiming payment link')
        let lightWasm = await WasmFactory.getInstance()
        let recipient = recipientAddress ? new PublicKey(recipientAddress) : this.publicKey
        let res = await withdrawFromPaymentLink({
            paymentLink,
            lightWasm,
            connection: this.connection,
            encryptionService: this.encryptionService,
        ...
        logger.debug(`Payment link claimed. Recipient ${recipient} received ${res.amount_in_lamports / LAMPORTS_PER_SOL} SOL, with ${res.fee_in_lamports / LAMPORTS_PER_SOL} SOL relayers fees`)
        this.isRuning = false
        return res
    }
