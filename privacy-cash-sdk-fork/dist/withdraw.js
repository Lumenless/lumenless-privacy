 ...
 */
export async function withdrawFromPaymentLink({ paymentLink, lightWasm, publicKey, connection, encryptionService, keyBasePath, recipient, referrer }) {
    const { blinding, amount, index, mintAddress } = paymentLink;
    let fee_in_lamports = Math.floor(amount * (await getConfig('withdraw_fee_rate')) + LAMPORTS_PER_SOL * (await getConfig('withdraw_rent_fee')));
    let amount_in_lamports = Math.floor(amount - fee_in_lamports);
    logger.debug('Claiming payment link...');
    logger.debug(`Payment link amount: ${amount} lamports`);
    logger.debug(`Fee: ${fee_in_lamports} lamports`);
    logger.debug(`Amount after fee: ${amount_in_lamports} lamports`);
    const { treeAccount, treeTokenAccount, globalConfigAccount } = getProgramAccounts();
    // Get current tree state
    const { root, nextIndex: currentNextIndex } = await queryRemoteTreeState();
    logger.debug(`Using tree root: ${root}`);
    logger.debug(`New UTXOs will be inserted at indices: ${currentNextIndex} and ${currentNextIndex + 1}`);
    // Use recipient's UTXO keypair (derived from their wallet)
    const utxoPrivateKeyV2 = encryptionService.getUtxoPrivateKeyV2();
    const utxoKeypairV2 = new UtxoKeypair(utxoPrivateKeyV2, lightWasm);
    // Reconstruct the input UTXO from payment link data + recipient's keypair
    const paymentLinkUtxo = new Utxo({
        lightWasm,
    ...
    logger.debug('Reconstructed payment link UTXO:');
    await paymentLinkUtxo.log();
    // Verify the UTXO exists by checking its commitment
    const commitment = await paymentLinkUtxo.getCommitment();
    logger.debug(`Payment link UTXO commitment: ${commitment}`);
    // Fetch Merkle proof to verify the UTXO exists
    let merkleProof;
    try {
        merkleProof = await fetchMerkleProof(commitment);
        logger.debug('Payment link UTXO found in Merkle tree');
    }
    catch (error) {
        throw new Error('Payment link UTXO not found. It may have already been claimed or the payment link data is invalid.');
    }
    // Create inputs: payment link UTXO + dummy UTXO
    const inputs = [
        paymentLinkUtxo,
        new Utxo({
            lightWasm,
    ...
    const inputNullifiers = await Promise.all(inputs.map(x => x.getNullifier()));
    const outputCommitments = await Promise.all(outputs.map(x => x.getCommitment()));
    // Encrypt the output UTXOs
    const encryptedOutput1 = encryptionService.encryptUtxo(outputs[0]);
    const encryptedOutput2 = encryptionService.encryptUtxo(outputs[1]);
    // Create the withdrawal ExtData
    const extData = {
        recipient,
        extAmount: new BN(extAmount),
        encryptedOutput1: encryptedOutput1,
