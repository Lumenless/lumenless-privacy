...
export class Keypair {
     ...
     */
    static fromPubkeyOnly(pubkey) {
        const kp = Object.create(Keypair.prototype);
        kp.pubkey = new BN(pubkey.toString());
        kp.privkey = new BN(0); // Placeholder - cannot be used for signing
        kp.lightWasm = undefined;
        return kp;
    }
    /**
    ...
    */
    sign(commitment, merklePath) {
        if (!this.lightWasm) {
            throw new Error('Cannot sign with a pubkey-only keypair');
        }
