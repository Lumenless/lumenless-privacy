...
export class EncryptionService {
   ...
   */
     ...
     * @returns The encrypted UTXO data as a Buffer
     */
    encryptUtxoForRecipient(utxo, recipientPublicKey) {
        ...
        const encrypted = nacl.box(message, nonce, Uint8Array.from(recipientPublicKey), ephemeralKeypair.secretKey);
        if (!encrypted) {
    ...
    decryptBoxUtxo(encryptedData) {
        ...
        const decrypted = nacl.box.open(Uint8Array.from(ciphertext), Uint8Array.from(nonce), Uint8Array.from(ephemeralPubkey), boxKeypair.secretKey);
        if (!decrypted) {
     ...
     */
    static isBoxEncrypted(encryptedData) {
     ...
     * @returns The encrypted data as a Buffer
     */
    encryptWithKey(data, encryptionKey) {
        // Convert string to Buffer if needed
        const dataBuffer = typeof data === 'string' ? Buffer.from(data) : data;
